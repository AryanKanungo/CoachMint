...
// ─────────────────────────────────────────────────────────────
// Screen-local palette (GitHub Dark)
// ─────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────
// Emergency Fund constant (screen-level — no DB column needed)
// ─────────────────────────────────────────────────────────────
...
class _TrackGoalsScreenState extends State<TrackGoalsScreen>
  ...
  /// All goals sorted: overdue first → soonest deadline → highest progress
  List<GoalModel> get _sorted {
    final list = List<GoalModel>.from(_goals);
    list.sort((a, b) {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          _err = "Couldn't load goals. Pull to refresh.";
          _loading = false;
        });
      }
    }
      ...
      builder: (_) => AlertDialog(
        ...
        actions: [
          TextButton(
              ...
              child: Text('Cancel',
                  style: GoogleFonts.dmSans(color: _textSec))),
          TextButton(
              onPressed: () async {
                Navigator.pop(context);
                // id is always a non-null String ('' for unsaved, uuid for saved)
                if (g.id.isNotEmpty) {
                  await _svc.deleteGoal(g.id);
                  _load();
                }
              },
              child: Text('Delete',
      ...
      children: [
        Text('Track Goals',
            style: GoogleFonts.dmSans(
                ...
                color: _textPri)),
        Text('${_goals.length} active goal${_goals.length == 1 ? '' : 's'}',
      ...
      slivers: [
        ...
        SliverPadding(
          ...
          sliver: SliverToBoxAdapter(
              child: _EmergencyFundCard(goals: _goals)),
        ),
...
// ═════════════════════════════════════════════════════════════
// _EmergencyFundCard
// Shows total savedAmount across ALL goals as a proxy for
// emergency fund progress (no special DB flag needed).
// ═════════════════════════════════════════════════════════════
class _EmergencyFundCard extends StatelessWidget {
  final List<GoalModel> goals;
  const _EmergencyFundCard({required this.goals});

  @override
  Widget build(BuildContext context) {
    // Use totalSaved across all goals as the EF proxy,
    // or 0 if user has no goals yet.
    final double saved =
        ...
        children: [
          ...
          ClipRRect(
            ...
            child: LinearProgressIndicator(
              ...
              valueColor:
              AlwaysStoppedAnimation(done ? _green : _cyan),
              minHeight: 7,
            ),
          ),
...
class _GoalCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final fillColor = complete ? _green : overdue ? _red : _blue;
    final bdrColor  = overdue
          ...
          Row(
            ...
            children: [
              Container(
                ...
                child:
                const Icon(Icons.savings_rounded, color: _blue, size: 18),
              ),
                ...
                children: [
                  ...
                  Text(_fmt(goal.savedAmount),
                      style: GoogleFonts.dmSans(
                          fontSize: 20,
                          fontWeight: FontWeight.w700,
                          color: complete ? _green : _textPri,
                          letterSpacing: -0.6)),
                ],
              ),
              const Spacer(),
...
// ─────────────────────────────────────────────────────────────
// _Stat — label / value row used in EF card
// ─────────────────────────────────────────────────────────────
...
// ═════════════════════════════════════════════════════════════
// _AddGoalSheet
// Fields match GoalModel exactly: title, targetAmount,
// savedAmount, deadline. No category / isEmergencyFund.
// ═════════════════════════════════════════════════════════════
...
class _AddGoalSheetState extends State<_AddGoalSheet> {
  ...
  bool _validate() {
    if (_titleCtrl.text.trim().isEmpty) {
      setState(() => _err = 'Please enter a goal title.');
      return false;
    }
    final t = _parse(_targetCtrl.text);
    if (t == null || t <= 0) {
      setState(() => _err = 'Enter a valid target amount (e.g. 25000).');
      return false;
    }
    final s = _parse(_savedCtrl.text);
    if (s == null) {
      setState(
              () => _err = 'Enter saved amount (use 0 if starting fresh).');
      return false;
    }
    if (s > t) {
      setState(() => _err = "Saved amount can't exceed target.");
      return false;
    }
    setState(() => _err = null);
    return true;
  ...
  Future<void> _submit() async {
    ...
    try {
      await widget.onSave(GoalModel(
        // id left as '' — DB will generate uuid
        userId:      '',         // GoalService stamps real uid on insert
...
class _Field extends StatelessWidget {
  ...
  Widget build(BuildContext context) => TextField(
    ...
    decoration: InputDecoration(
      hintText: hint,
      hintStyle:
      GoogleFonts.dmSans(fontSize: 14, color: _textSec),
      filled: true,
      fillColor: _bg,
      isDense: true,
  ...
  Widget build(BuildContext context) => Padding(
    ...
    child: Column(
      ...
      children: [
        ...
        const SizedBox(height: 8),
        Text(
            'Goals give your savings direction.\nTap + to set your first target.',
            style: GoogleFonts.dmSans(
