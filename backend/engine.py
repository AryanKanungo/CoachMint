...
# ... (Keep all your Dataclasses, build_input, upsert_snapshot, and run_engine math exactly as you sent them) ...

def execute_full_pipeline(user_id: str, db):
    """
    MODULAR ORCHESTRATOR:
    This is the single source of truth that runs both Phase 1 (Engine) 
    and Phase 2 (Predictor). 
    """
    from predictor import run_prediction # Imported here to avoid circular imports

    # 1. Run Ground Truth Engine (Phase 1)
    engine_input = build_input(user_id, db)
    engine_output = run_engine(engine_input)
    upsert_snapshot(user_id, engine_output, db)

    # 2. Run Time-Series Predictor (Phase 2)
